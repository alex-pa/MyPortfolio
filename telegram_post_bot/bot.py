...
post_dict = {}
# channel's id to send final results to
...
def test_callback(call):
    ...
    if call.data == "edit_before_sending":
        ...
        post = post_dict[call.message.chat.id]
        msg = bot.send_message(call.message.chat.id, post.print_post(), parse_mode="Markdown")
...
def process_phonetic_transcription(message):
    if message.text != 'Skip':
        ...
        phonetic_transcription = phonetic_transcription.replace('ˈ', "'")
        post = post_dict[message.chat.id]
        word = post.words[-1]
...
def process_edited_post(message):
    post = post_dict[message.chat.id]
    bot.forward_message(storage_channel_id, message.chat.id, message.message_id)
    links = post.print_links()
    if links:
        bot.send_message(storage_channel_id, links, parse_mode="Markdown")
